...
function TopbarCo() {
  ...
  const handleSearch = async () => {
    try {
      ...
      setShowSearchResults(true); // Montrer les résultats après la recherche
    } catch (error) {
  ...
  const handleClickOutside = (e) => {
    if (searchRef.current && !searchRef.current.contains(e.target)) {
      setShowSearchResults(false); // Cacher les résultats si le clic est en dehors de la div mainSearch
    }
  };
